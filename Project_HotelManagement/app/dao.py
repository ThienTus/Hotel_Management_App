...
def send_message_twilio(message):
    ...
    account_sid = 'ACc7bdd55f7a55a8343b37cf7ce4c7231b'
    auth_token = '11df65dc0d22042b176cc011e0d83f6d'
    client = Client(account_sid, auth_token)

    # Đảm bảo bạn sử dụng dấu ngoặc nhọn `{}` thay vì dấu ngoặc tròn `()`
    verification = client.verify \
        .services('VA78b382023f90201246c2d2bd62afd37a') \
        .verifications \
        .create(to='+840375290878', channel='sms')
...
def send_gmail(receive_email=None, subject=None, message=None):
    if receive_email and subject and message:
        ...
        server.login(email, "Toiyeuyasuo123")
...
Congratulations! Your booking at Tripple T Hotel is confirmed. We look forward to hosting you and providing a memorable stay.
        ...
        with app.app_context():
            ...
            if is_calculate:
                ...
                twilio_thread = threading.Thread(target=send_message_twilio,
                                                 args=(
                                                     "Thanks for using services at Tripple T hotel and see you next time!",))
                content = (f'You have paid {rc.total_price} VND for your room at '
                           f'Tripple T hotel, thanks for all!')
                gmail_thread = threading.Thread(target=send_gmail,
                                                args=(
                                                    emails[0],
                                                    f"YOU HAVE PAID {total_price} FOR YOUR RECEIPT AT TRIPPLE T HOTEL!",
                                                    content))
